...
\graphicspath{{../figures/}{../figures/pub/}}
...
Monte Carlo simulation provides the gold standard for photon transport in inhomogeneous geometries, exactly solving the radiative transport equation in anatomically realistic head models. Previous deep-brain fNIRS simulations have been limited by simplified geometries, insufficient photon statistics, or idealized noise models.
...
An anatomically realistic seven-tissue head model was generated from the MNI152 standard brain (1~mm isotropic T1 atlas) using iso2mesh~v2.0 with TetGen~v1.6. Tissue classes were: scalp, skull, cerebrospinal fluid (CSF), gray matter, white matter, amygdala (bilateral, merged into one label), and lateral ventricles. Mesh quality constraints enforced a maximum circumradius-to-inradius ratio of~1.5 and a maximum tetrahedral volume of~5~mm$^3$, yielding 347,412 tetrahedra with minimum dihedral angle $\geq 15^\circ$. The right amygdala centroid lies at MNI coordinate $(24.0,\,-2.0,\,-18.0)$~mm, 60.8~mm from the closest scalp vertex.

Optical properties at 730~nm and 850~nm were assigned per tissue from published spectroscopy data (Table~\ref{tab:optical}). Baseline hemoglobin concentrations (HbO~=~60~$\mu$M, HbR~=~40~$\mu$M) set the background absorption via
\begin{equation}
    \mu_a(\lambda) = \ln(10)\bigl[\varepsilon_\mathrm{HbO}(\lambda)\,c_\mathrm{HbO}
    + \varepsilon_\mathrm{HbR}(\lambda)\,c_\mathrm{HbR}\bigr],
    \label{eq:mua_bg}
\end{equation}
where $\varepsilon_j(\lambda)$ (cm$^{-1}\mu$M$^{-1}$) are molar extinction coefficients from the Prahl tabulation, and $c_j$ ($\mu$M) are chromophore concentrations.
...
Scattering deflections were sampled from the Henyey--Greenstein (HG) phase function:
\begin{equation}
...
Tetrahedral face intersections were located with the M\"{o}ller--Trumbore ray--triangle algorithm (numerical tolerance $\pm10^{-6}$~mm). At each interface between tissues with refractive indices $n_1$ and $n_2$, the incident angle $\theta_i$ satisfies Snell's law:
\begin{equation}
...
The MMC solver was implemented in CUDA~12.x and executed on an NVIDIA RTX~5090 GPU (16{,}384 CUDA cores, 32~GB GDDR7). A bounding-volume hierarchy (BVH) over tetrahedral elements enabled $O(\log N_\mathrm{tet})$ ray--element intersection queries. Independent photon trajectories ran on individual CUDA threads; fluence accumulation used \texttt{atomicAdd} for thread-safe weight deposition. Simulation throughput reached $6.0 \times 10^6$~photons/s, completing $10^{10}$~photons per wavelength in approximately 28~minutes.

\subsection{Optode Configuration}

A single optical source (600~$\mu$m core FT8 fiber, 1~W continuous-wave, dual-wavelength 730~nm/850~nm) was placed at MNI coordinate $(24.4,\,-10.2,\,-71.0)$~mm, the closest scalp surface point to the right amygdala centroid, giving an anatomical minimum source--amygdala distance of 60.8~mm. Laser power was confirmed below the ANSI~Z136.1 maximum permissible skin exposure ($400$~mW/cm$^2$ for $>10$~s at $\lambda > 700$~nm) for this fiber diameter.
...
\begin{equation}
    ...
    \label{eq:ngate}
\end{equation}
where $P_\lambda/(h\nu_\lambda)$ is the source photon rate, $\eta_\mathrm{det}$ is detector quantum efficiency, $T_\mathrm{int}$ is integration time (120~s baseline), $N_\mathrm{MC,gate}/N_\mathrm{MC}$ is the simulated gate fraction, and $A_\mathrm{det}/A_\mathrm{scalp}$ is the geometric coupling ratio.
...
The scalp contribution to a long-SDS measurement is estimated via a collocated short-SDS reference:
\begin{equation}
    \Phi_\mathrm{corr}(t) = \Phi_\mathrm{long}(t) - \hat{\beta}\,\Phi_\mathrm{short}(t),
    \label{eq:ssr}
\end{equation}
where $\hat{\beta}$ is the ordinary least-squares coefficient estimated over a resting baseline. Regression was performed independently at each wavelength. For TD measurements, SSR was applied independently within each 10~ps TPSF time bin before gating.
...
\end{equation}
consistent with reported physiolo gical noise floors in long-SDS TD fNIRS.
...
\begin{table}[H]
\centering
\caption{Minimum detectable $\Delta$HbO at 850~nm (5~ns$+$ gate, 120~s integration).}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{11_cw_vs_td_sensitivity.png}
\caption{Scalp-to-amygdala contamination ratio vs.\ time gate and SDS. CW (leftmost) shows ratios of $10^5$--$10^7$. Gating beyond 5~ns stabilises the ratio at $3700$--$13{,}000\times$, a $25$--$4300\times$ improvement over CW.}
\label{fig:cw_vs_td}
\end{figure}
...
Shot-noise-limited sensitivity scales as $1/\sqrt{T_\mathrm{int}}$. Table~\ref{tab:int_time} shows the minimum detectable $\Delta$HbO for three detectors across integration times from 1~s to 120~s.
